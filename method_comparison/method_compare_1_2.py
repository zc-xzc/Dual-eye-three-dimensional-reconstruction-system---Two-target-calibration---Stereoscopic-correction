...
# 设置全局样式
node_style = {'shape': 'box', 'fontname': font_name, 'fontsize': '9', 'style': 'filled', 'fillcolor': 'lightgray'}
edge_style = {'fontname': font_name, 'fontsize': '8'}
graph_style = {'rankdir': 'LR', 'size': '12,6', 'bgcolor': 'white', 'fontname': font_name, 'fontsize': '12'}
...
def draw_flowchart_1():
    ...
    dot.node('图像识别', label='图像识别：采集到的图像信号发送至\nJetson Nano上位机进行垃圾类型识别')
    dot.node('控制信号发送', label='控制信号发送：上位机根据识别结果\n向控制下位机发送控制信号')
    ...
    # 保存并渲染为PDF
    dot.render('flowchart_1', format='pdf', view=True)
...
def draw_flowchart_2():
    ...
    # 保存并渲染为PDF
    dot.render('flowchart_2', format='pdf', view=True)
...
def draw_flowchart_3():
    ...
    # 保存并渲染为PDF
    dot.render('flowchart_3', format='pdf', view=True)
