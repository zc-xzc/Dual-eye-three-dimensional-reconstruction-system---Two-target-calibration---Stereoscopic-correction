...
def aruco_detect(frame):
    ...
    parameters = cv2.aruco.DetectorParameters()  # 修改为新的方法
    '''
    detectMarkers(...)
...
def generate_3D_point(grid_size, distance, size=(15, 13)):  # 修改为新的尺寸
...
# size:标定板marker的行数，列数 eg：(15, 13)
def camera_calibrate(grid_size, distance, frame, size=(15, 13)):  # 修改为新的尺寸
    h, w, c = frame.shape
    # 生成标定板的3D point
    ...
    # 利用3D和2D对应点计算变换矩阵
    (success, rotation_vector, translation_v) = cv2.solvePnP(point_3d, corners_2d, mtx, dist, flags=cv2.SOLVEPNP_ITERATIVE)
    if success:
...
def calculate_rejection_error(corners_2d, point_3d, rotation_v, translation_v, camera_matrix, distortion_coefficient):
    ...
    for i in range(len(point_3d)):
        imgpoints2, _ = cv2.projectPoints(point_3d[i], np.array(rotation_v), np.array(translation_v), camera_matrix,
                                          distortion_coefficient)
        imgpoints2 = imgpoints2.flatten()
        error = cv2.norm(corners_2d[i, :], imgpoints2, cv2.NORM_L2)
        total_error += error
    return total_error
