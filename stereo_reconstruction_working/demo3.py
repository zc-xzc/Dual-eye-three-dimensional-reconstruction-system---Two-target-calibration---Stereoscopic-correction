...
class StereoDepth(object):
    ...
    def test_pair_image_file(self, left_file, right_file):
        ...
        if not os.path.exists(left_file) or not os.path.exists(right_file):
            print(f"Error: One or both image files do not exist: {left_file}, {right_file}")
            return

        frameR = cv2.imread(left_file)
        frameL = cv2.imread(right_file)

        if frameR is None or frameL is None:
            print(f"Error: One or both image files could not be read: {left_file}, {right_file}")
            return
    ...
    def task(self, frameL, frameR, waitKey=5):
        ...
        if frameL is None or frameR is None:
            print("Error: One or both frames are empty.")
            return
        ...
        if dispL is None or not np.any(dispL):
            print("Warning: Disparity map is empty or invalid.")
            return
        points_3d = self.get_3dpoints(disparity=dispL, Q=self.camera_config["Q"])
        if points_3d is None or not np.any(points_3d):
            print("Warning: 3D points are empty or invalid.")
            return
    ...
    def show_3dcloud_for_open3d(self, frameL, frameR, points_3d):
        ...
        if self.use_open3d:
            ...
            if np.all(depth == 0):
                print("Warning: Depth values are all zero.")
                return
