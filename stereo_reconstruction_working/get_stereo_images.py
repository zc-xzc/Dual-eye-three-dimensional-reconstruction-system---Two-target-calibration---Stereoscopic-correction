    ...
    def capture1(self, video, save_dir):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                print("save image:{:1=3d}".format(i))
                cv2.imwrite(os.path.join(save_dir, "left_{:1=3d}.png".format(i)), frameL)
                cv2.imwrite(os.path.join(save_dir, "right_{:1=3d}.png".format(i)), frameR)
                i += 1
            writerL.write(frameL)
            writerR.write(frameR)
        cap.release()
